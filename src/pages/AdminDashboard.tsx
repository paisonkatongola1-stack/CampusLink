...
import { updateListingStatus } from '../utils/firebaseUtils';
import { useState } from 'react';

const AdminDashboard = () => {
  const [activeTab, setActiveTab] = useState<'accommodation' | 'marketplace' | 'events'>('accommodation');
  const [pendingItems, setPendingItems] = useState([
    { id: 'a1', title: "Riverside Shared Room", user: "Landlord: John B.", type: "accommodation" },
    { id: 'e1', title: "Zambia Tech Expo", user: "Organizer: ICT Union", type: "events" },
    { id: 'm1', title: "HP Laptop G8 Pro", user: "Student: Mwaka M.", type: "marketplace" }
  ]);

  const handleAction = async (id: string, collection: string, status: 'approved' | 'rejected') => {
    try {
      // In a real app, this would update Firestore
      // await updateListingStatus(collection, id, status);

      // Update local state for demo
      setPendingItems(prev => prev.filter(item => item.id !== id));
      alert(`${collection} item ${status} successfully!`);
    } catch (error) {
           ...
           <motion.div variants={fadeInUp} className="space-y-8">
              ...
              <div className="glass overflow-hidden rounded-[2.5rem] border border-white/5 shadow-2xl relative">
                 ...
                 <div className="p-8 space-y-6">
                    {pendingItems.filter(i => i.type === activeTab).length > 0 ? (
                      pendingItems.filter(i => i.type === activeTab).map((item) => (
                      ...
                      <div className="py-10 text-center text-gray-600 text-[10px] font-black uppercase tracking-widest">No pending {activeTab} approvals</div>
                    )}
                 </div>
              </div>
           </motion.div>
