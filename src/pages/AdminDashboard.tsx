...
const AdminDashboard = () => {
  const [activeTab, setActiveTab] = useState<'accommodation' | 'marketplace' | 'events'>('accommodation');

  const getCollectionName = (tab: string) => {
    return tab === 'accommodation' ? 'accommodation' : tab === 'marketplace' ? 'marketplace' : 'events';
  };

  const { data: pendingItems } = useCollection<any>(getCollectionName(activeTab), [
    where('status', '==', 'pending')
  ]);

  const handleApproval = async (id: string, approved: boolean) => {
    const itemRef = doc(db, getCollectionName(activeTab), id);
                 ...
                 <div className="p-8 space-y-6">
                    {pendingItems.length > 0 ? pendingItems.map((item, i) => (
                      <div key={item.id} className="flex items-center justify-between p-5 bg-white/3 rounded-[1.5rem] border border-white/5 hover:border-primary/20 transition-all group">
                        <div>
                          <div className="font-bold text-sm tracking-tight group-hover:text-primary transition-colors">{item.title || item.name || item.role}</div>
                          <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1">{item.location || item.university}</div>
                        </div>
                        <div className="flex space-x-3">
                           <button onClick={() => handleApproval(item.id, true)} className="p-3 bg-green-500/10 text-green-500 rounded-xl hover:bg-green-500 hover:text-white transition-all shadow-lg shadow-green-500/10"><CheckCircle size={18} strokeWidth={2.5} /></button>
                           <button onClick={() => handleApproval(item.id, false)} className="p-3 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-lg shadow-red-500/10"><AlertCircle size={18} strokeWidth={2.5} /></button>
                        </div>
                      </div>
                    )) : (
                      <div className="text-center py-10 text-gray-600 text-[10px] font-black uppercase tracking-[0.3em]">
